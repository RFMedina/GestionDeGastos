...
public class GeneracionDatos implements IGeneracionDatos {
    ...
    public void generarDatos() {
        if (usuarios.findAll().iterator().hasNext() == false) {
            ...
            //6 7 y 8
            movimientos.save(new Movimiento(13, "Alimentacion", Date.from(Instant.now()), "Compra de 5 yatekomo", -12d, usuarioGrupos.getById(5)));
            movimientos.save(new Movimiento(14, "Regalo", Date.from(Instant.now()), "Propina de la abuela", 30d, usuarioGrupos.getById(4)));
            movimientos.save(new Movimiento(15, "Transporte", Date.from(Instant.now()), "Monedas en el suelo", 1.5d, usuarioGrupos.getById(5)));
            movimientos.save(new Movimiento(16, "Regalo", Date.from(Instant.now()), "Libro de Stephen King para Paloma", -11.9, usuarioGrupos.getById(6)));
            movimientos.save(new Movimiento(17, "Entretenimiento", Date.from(Instant.now()), "Cascos New Skill Kimera V2", -45.50, usuarioGrupos.getById(4)));
            movimientos.save(new Movimiento(18, "Comunicacion", Date.from(Instant.now()), "Resolución juicio del bar", 600.65, usuarioGrupos.getById(4)));
            movimientos.save(new Movimiento(19, "Higiene", Date.from(Instant.now()), "Cremas, pasta de dientes etc", -40.32, usuarioGrupos.getById(6)));
            movimientos.save(new Movimiento(20, "Entretenimiento", Date.from(Instant.now()), "Humble Bundle", -0.9, usuarioGrupos.getById(5)));
            movimientos.save(new Movimiento(21, "Regalo", Date.from(Instant.now()), "Tarjeta reagalo hipercor", 5.5, usuarioGrupos.getById(4)));
            movimientos.save(new Movimiento(22, "Alimentacion", Date.from(Instant.now()), "Carro de la compra", -60d, usuarioGrupos.getById(5)));
            movimientos.save(new Movimiento(23, "Transporte", Date.from(Instant.now()), "Mas viajes de tranvia", -3d, usuarioGrupos.getById(6)));
            movimientos.save(new Movimiento(24, "Alimentacion", Date.from(Instant.now()), "Cena UDON", -30.5, usuarioGrupos.getById(4)));
